   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @Noctophile")
            return 1
      except UserNotParticipant:
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://envs.sh/BcS.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
      except Exception:
         await message.reply_text("Something Went Wrong. Contact us @Noctophile...")
         return 1
...
async def progress_callback(current, total, progress_message):
    ...
    f"╰──────────────────╯\n\n"
    f"**__Powered by Bhartiyee Leech__**"
        )
